...
- Global defaults initialize new documents, with optional source defaults taking precedence. Changes never mutate existing document choices or reschedule their automation, including during loading, refresh, source reconnect and Session switching. The global default controls share an independent observable subscription and appear vertically under More. The same button reads Collapse while the defaults are expanded and hides them when selected again. There are no resource inheritance or reset actions. Closing the last view and reopening uses the latest defaults.
...
- Hover, keyboard focus or a touch/click on the status exposes a vertical Update, Save, Differences and More stack. Update and Save have left checkboxes for their document automation policies. Operations do not reveal action buttons or schedule completion-feedback timers. Escape and outside clicks dismiss the controls; active hover/focus keeps them open until dismissal. Only controls intercept editor pointer input. Failures, paused automation and conflict-resolution actions remain available independently of the overlay.
- Differences has one toolbar entry that becomes Back to editor. Comparison mode replaces the visible editor area with read-only inline diffs against Base, showing red deletions and green insertions. Sides equal to Base are omitted, identical Local and Source text appear once, and Base has no separate column. The view follows local edits and the latest source observation. After synchronization it remains in comparison mode with the current full text. The normal editor stays mounted and hidden, retaining selection, scroll and undo. The lazy editor package owns `@codemirror/merge`; accept/reject merge controls are disabled.
...
- `VIEWER-008`: More and Collapse use one button; Differences appears only in the floating toolbar. Inline comparisons update from document observations without modifying source or local text, omit unchanged/repeated sides, retain mode through synchronization, and return to the same mounted editor.
