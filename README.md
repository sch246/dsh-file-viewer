...
Hover, focus or tap the status to reveal Update, Save, Differences and More. The checkbox to the left of Update or Save controls that document's automation policy; the button performs the manual action. More reveals the two default-policy checkboxes and changes its own label to Collapse; selecting it again hides those defaults. Escape and outside clicks dismiss the controls. Operation activity does not open the controls, while failures, paused automation and conflict-resolution actions stay accessible.
...
Differences replaces the visible editor area with read-only inline comparisons against the common base: deleted text is red and inserted text is green. It displays only changed sides, combines identical Local and Source text, and never adds a separate Base column. Changes follow the current local text and the source's latest observation. With no differences, the current full text remains visible. The mode stays open until Back to editor is selected; the normal editor remains mounted with its selection, scroll and undo state. Diff controls cannot accept or reject changes automatically. Overwrite source and Discard local require separate confirmation. `Ctrl+S` or `Cmd+S` requests an immediate save. A dirty close also requires confirmation.
