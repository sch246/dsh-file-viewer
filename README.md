...
## Native document and media readers

PDF, audio and video are built-in lazy handlers alongside image viewing. Open a matching file normally or select its reader in Open with. The browser supplies PDF controls and audio/video playback; media does not autoplay. Download and reload remain available, with visible errors for unsupported formats. Build user-files ^0.1.11 before viewer ^0.1.7; no additional Bundle is needed. The [package reference](packages/dsh-file-viewer/README.md#pdf-audio-and-video) owns the streaming interface and browser limitations.
