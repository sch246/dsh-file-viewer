...
Text automation preferences resolve global defaults, optional source defaults and explicit resource overrides. Capability checks gate execution without rewriting a saved preference, and reset removes the resource override. The version-1 Base/Local browser draft format remains authoritative for recovery.
