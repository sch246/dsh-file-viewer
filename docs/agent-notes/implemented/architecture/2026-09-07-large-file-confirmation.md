...
The filesystem adapter maps the shared provider’s typed `user-files/confirmation-required` error. Provider configuration remains authoritative for thresholds, byte limits and file publication. This introduces no manager, sidebar or Host responsibility. Approval, growth and cancellation are observable product conditions in STATE; retained provider checks address read integrity.
