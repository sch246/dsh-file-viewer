...
Filesystem stream resources optionally supply `ResourceStream.download(signal, onProgress)`, called directly from the Parallel download button to preserve save-picker activation. The button shows percent/cancellation and a straight black progress bar. An ordinary download link remains alongside it. Custom transfers require user-files ^0.1.12 and stop with their renderer; native PDF-toolbar downloads remain outside this helper. See user-files' download reference for compatibility, configured concurrency and version checks.
