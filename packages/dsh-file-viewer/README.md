...
Large-file edits publish Local text immediately and check its complete hash after input remains idle for `largeEditCheckDelayMs` (300 ms by default). One background check runs per document; stale completions cannot replace a later edit. Manual Save verifies the captured latest text immediately. See [loading, retry and editing configuration](../../README.md#synchronize-a-document).
